...
def next_iter(u_values,
    ...
    for (u,
        ...
        nearest_notch_free_sum =0 
        for (neighbour_u, 
             neighbour_delta_free,
             neighbour_notch_free) in zip(nearest_neighbours_u,
                                          nearest_neighbours_delta_free,
                                          nearest_neighbours_notch_free):
        ...
        delta_notch_trans = Ktrans/6*delta_free*nearest_notch_free_sum
        #total delta and total notch
        #basic behaviour
        if not noDelta:
            new_delta_tot = delta_tot + ffact_Notch*(u*(u>0)-delta_tot) - ffact_signal * k_signal * delta_notch_trans 
        else:
            new_delta_tot = delta_tot + ffact_Notch*(0-delta_tot)
        new_notch_tot = notch_tot + ffact_Notch*(N_low-notch_tot) - ffact_signal * k_signal * notch_delta_trans 
        new_signal =  signal + ffact_signal*(k_signal *notch_delta_trans - signal)
        #new_signal = signal
        if extraSignal:
           new_signal += extraSignalValue
        #extra production if high notch signalling
        new_notch_tot += ffact_Notch *(N_high-N_low)* (1+np.tanh((new_signal-Sstar)/alphaS))/2.0
        new_u= u+ffact_u*(-u*(u-1)*(u-r)-JI *(new_signal>S0)*(u>0)* new_signal + JA  *(nearest_u_sum - 6 *u ))
        if extraVeinActivation:
            new_u+= -ffact_u* extraVeinActivationValue * (u-1)
        #Notch signalling reporter
        new_notch_reporter = notch_reporter + ffact_reporter*(new_signal-notch_reporter)
        #store result
        new_delta_tot_values.append(new_delta_tot)
        new_notch_tot_values.append(new_notch_tot )
        new_signal_values.append(new_signal)
...
def run_sim(cells, 
    ...
    for t in range(N_times):
       ...
       signal_values = np.array([c.signal for c in cells.values()])
    ...
    #then update other quantities.
       ...
       nearest_neighbour_delta_free_values = np.array([[cells[c2].delta_free
                                                         ...
                                                         c2 in c.neighbours]
                                                       for c in cells.values()] )
       nearest_neighbour_notch_free_values = np.array([[cells[c2].notch_free
                                                         for
                                                         c2 in c.neighbours]
                                                       for c in cells.values()] )
